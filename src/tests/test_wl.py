...
def val_step_triplet(model, loader_pos, loader_neg, device):
    ...
    return trip_acc, all_sim_pos, all_sim_neg
...
def main():
    ...
    loader_test_pos = DataLoader(pairs_pos_te, batch_size=1, follow_batch=['x_s', 'x_t'], shuffle = False)
    loader_test_neg = DataLoader(pairs_neg_te, batch_size=1, follow_batch=['x_s', 'x_t'], shuffle = False)
    ...
    val_acc, pos_geds, neg_geds = val_step_triplet(model, loader_test_pos, loader_test_neg, args.device)
    ...
    '''
    val_auroc, pos_geds, neg_geds = val_step(model, loader_train, args.device)
    val_acc, pos_geds, neg_geds = val_step_triplet(model, loader_train_pos, loader_train_neg, args.device)

    print(val_auroc, val_acc)

    plt.figure()
    plt.hist(pos_geds.cpu().numpy(), alpha=0.4)
    plt.hist(neg_geds.cpu().numpy(), alpha=0.4)
    plt.tight_layout()
    plt.savefig(args.basename+"-train.png")   
    '''   
