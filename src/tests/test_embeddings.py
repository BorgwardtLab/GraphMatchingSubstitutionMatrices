...
def main():
    ...
    embs = np.load(args.ckp)
    pdbids = embs.files

    dim = embs[pdbids[0]].shape
    print(dim)
    ...
    for pair in pairs_pos_te:
        try: emb1 = embs[pair[0]]
        except: emb1 = np.zeros(dim)
        try: emb2 = embs[pair[1]]
        except: emb2 = np.zeros(dim)
        dist_pos.append( np.linalg.norm(emb1 - emb2) )
    ...
    for pair in pairs_neg_te:
        ...
        dist_neg.append( np.linalg.norm(emb1 - emb2) )
    ...
    plt.figure()
    bins = np.histogram(dist.numpy(), bins=40, range=(0.0,10.0))[1] #get the bin edges
    plt.hist(np.array(dist_pos), bins, alpha=0.4)
    plt.hist(np.array(dist_neg), bins, alpha=0.4)
    plt.tight_layout()
